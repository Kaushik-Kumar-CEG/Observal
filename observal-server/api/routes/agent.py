...
async def _load_agent(db: AsyncSession, agent_id: str) -> Agent:
    return await resolve_prefix_id(Agent, agent_id, db, load_options=_agent_load_options)
...
async def install_agent(
    ...
    try:
        agent = await resolve_prefix_id(
            Agent, agent_id, db, extra_conditions=[Agent.status == AgentStatus.active], load_options=_agent_load_options
        )
